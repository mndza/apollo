...
{
	switch (target) {
		case SPI_FPGA_JTAG:  return SERCOM0;
		default:             return NULL;
	}
}
...
static void _spi_configure_pinmux(spi_target_t target, bool use_for_spi)
{
	switch (target) { 
		...
		// FPGA JTAG connection -- configure PA14 (TDI), PA15 (TCK), and PA10 (TDO).
		case SPI_FPGA_JTAG:
			if (use_for_spi) {
	 ...
	 uint8_t clock_polarity, uint8_t clock_phase)
{
	...
	sercom->SPI.CTRLA.reg =
		...
		SERCOM_SPI_CTRLA_DOPO(0)          |  // use PAD0 as MOSI, PAD1 as SCK
		SERCOM_SPI_CTRLA_DIPO(2)          |  // use PAD2 as MISO
