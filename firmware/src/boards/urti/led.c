...
void led_init(void)
{
    uint8_t pins[] = { LED_A, LED_B, LED_C };
...
void led_set_pattern(led_pattern_t pattern)
{
    led_pattern = pattern;
    leds_off();
    // Values of 0 to 7 should be set immediately as static patterns.
    if (led_pattern < 7) {
...
void led_task(void)
{
  ...
  // Values of 0 to 7 define static patterns only.
  if (led_pattern < 7) {
    return;
  }

  // When the device is idle, use the following scheme for LEDs:
  // - LED A: FPGA allowed online (indicates PROGRAM toggle)
  // - LED B: USB switched to FPGA
  // - LED C: flashing patterns (e.g. fault indication)
  if (led_pattern == LED_IDLE) {
    led_set(LED_A, fpga_is_online());
    led_set(LED_B, fpga_controls_usb_port());
    led_set(LED_C, false);
    return;
  }
